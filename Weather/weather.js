...
function searchResults (city) {
    fetch(`${api.base}q=${city}&lang=${api.lang}&units=${api.units}&APPID=${api.key}`)
        .then (res => {
            if (!res.ok){
...
function coordResults(lat, long) {
    fetch(`${api.base}lat=${lat}&lon=${long}&lang=${api.lang}&units=${api.units}&APPID=${api.key}`)
        .then(response => {
            if (!response.ok) {
...
function dateBuilder(d) {
    ...
    let day = d.getDay(days);
    let date = d.getDate(months);
    let month = d.getMonth();
    let year = d.getFullYear();

    return document.getElementById("Date").textContent = `${day}, ${date}, ${month}, ${year}`;
}
